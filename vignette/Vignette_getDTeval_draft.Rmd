...
This vignette aims to provide an introduction to the  getDTeval and an approach to implementing programmaticly designed coding statements without compromising on the runtime performance. Familiarity with the syntax of the data.table and dplyr packages would be useful, but it is not essential to follow this vignette. 
...
- **coding.statement.as** determines whether the coding statements provided as outputs are returned as expression objects (return.as = "expression") or as character values (return.as = "character", which is the default).

- **eval.type** allows user to specify if they want the coding statement to be evaluated in its current form or in optimized form with get() and eval() translations involved. 

### Examples of getDTeval()

getDTeval function specifies an option to the user in chosing the mode of the output. The output could be the optimized coding statement or the resulting calculation or a combination of both. This gives an added benefit of verifying the intended original statement. 

For example, calculating mean income by gender for the survey respondents using getDTeval can be carried out as follows where 1a outputs the optimized statement, 1b outputs the actual results and 1c presents us with both the code and the calculation. 

### Example 1:  Grouped Mean in data.table with get()
...
benchmark.getDTeval function facilitates the comparison of running time complexity of different forms of a coding statement.  The original statement's performance will be compared to the running time after the statement is translated using getDTeval().
...
We will be calculating mean awareness of the survey's participants by region.  The benchmark.getDTeval() function will be used to compare the running times of the original programmatic coding statement to the translated statement.  This comparison also evaluates the time of the optimized statement, which would use the translated statement without the time required to translate it.
...
## Benefits/Applications of getDTeval

getDTeval offers a plethora of benefits to bridge the gap between the code readability and code runtime execution. These benefits are as follows:

- minimize the running time complexity of the algorithms
- use the available computing resources efficiently
- are easy for the analyst to implement
- provide greater familiarity and readability to those who will review and use the code
- maintain flexibility for streamlined iterations and revision

Now we will dive into specific use cases which make getDTeval extremely handy to improve the current coding designs in popular packages such as data.table and dplyr.
...
But with getDTeval, the translation of the above statement allows us to get the desired results without using any other method to resolve the above issue. 
